...
class HACAModel(nn.Module):
    ...
    def __init__(self, config, device, batch_size, max_caption_length):
        ...
        self.audio_encoder = encoder(config["audio_input_size"], config["audio_hidden_size"],
        config["chunk_size"]["audio"], device)
        self.visual_encoder = encoder(config["visual_input_size"], config["visual_hidden_size"],
        config["chunk_size"]["visual"], device)

        # Decoders (Global & Local)
        encoder_output_size = {"audio" : 2*config["audio_hidden_size"]["low"], "visual" : 2*config["visual_hidden_size"]["low"]}
        # Local Decoder
        self.local_decoder = decoder(config["local_input_size"], config["local_hidden_size"], False,
        encoder_output_size, device, config["embedding_size"], config["vocab_size"], config["global_hidden_size"])
        encoder_output_size = {"audio" : config["audio_hidden_size"]["high"], "visual" : config["visual_hidden_size"]["high"]}

        # Global Decoder
        self.global_decoder = decoder(config["global_input_size"], config["global_hidden_size"], True,
        encoder_output_size,device, config["embedding_size"], config["vocab_size"], None)
